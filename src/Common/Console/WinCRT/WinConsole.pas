...
{ Copyright:  (c) 2006 Rudy Velthuis                                        }
...
procedure Sound(Frequency: Smallint); overload;

// Plays a sound at the given frequency (in Hz) and duration (in ms).
procedure Sound(Frequency, Duration: Smallint); overload;
...
var
  ExtendedChar: Char = #0;
  Win32Platform: Cardinal = VER_PLATFORM_WIN32_NT;
...
function Validate(var X1, Y1, X2, Y2: Integer): Boolean;
begin
...
procedure WriteText(Line: PChar; Len: Integer);
var
  NumWritten: DWORD;
begin
  SetConsoleTextAttribute(StdOut, TextAttr);
  WriteConsole(StdOut, Line, Len, NumWritten, nil);
end;
...
var
  ...
  procedure CharToWriteBuffer(C: Char);
  begin
    WritePtr^ := C;
    Inc(WritePtr);
    Dec(DistanceToEdge);
  ...
  begin
    WritePtr^ := #0;
    WriteText(PChar(Line), WritePtr - PChar(Line));
  ...
  var
    Num, I: Integer;
  begin
    Num := 8 - (WritePtr - PChar(Line)) mod 8;
...
procedure Sound(Frequency: Smallint);
begin
...
procedure Delay(Millisecs: Integer);
begin
  if (Win32Platform = VER_PLATFORM_WIN32_NT) and (SoundState = ssPending) then
  begin
...
procedure Sound(Frequency, Duration: Smallint);
begin
  if Frequency >= 37 then
  begin
    if Win32Platform = VER_PLATFORM_WIN32_NT then
