...
procedure TjanRecordset.SaveToFile(filename: string);
var
  gen:TStringList;
  i,c:integer;
...
begin
  ...
  for i:=0 to FFields.count-1 do
   if result='' then
     result:=TjanSQLRecordField(FFields[i]).value
   else
     result:=result+';'+TjanSQLRecordField(FFields[i]).value;
