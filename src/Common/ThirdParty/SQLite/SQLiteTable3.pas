...

{
	...
	*Const'd a few parameters that had strings.
	*Extrated the database loading into a new method, Connect
	*Create doesn't load the database anymore
	*Added THandle to hold the library information.
}
...
begin
	...
	if fInTrans then
		ExecSQL('ROLLBACK;'); //assume rollback
...
function TSQLiteDatabase.Connect(const FileName: string): boolean;
var
	Msg : PChar;
	iResult : integer;
begin
...
begin
	...
	if sqlite3_errcode(fDB) <> SQLITE_OK then
		Msg := sqlite3_errmsg(fDB);
...
begin
	try
		...
		if Sqlite3_Prepare(fDB, PChar(SQL), -1, Stmt, NextSQLStatement) <>
			SQLITE_OK then
...
begin
	...
	begin
		...
		begin
			...
			case pInteger(fColTypes[iColNo])^ of
				dtBlob:
...
begin
	...
	//methods
	thistype := pInteger(fColTypes[I])^;

	case thistype of
		dtStr:
			begin
				thisvalue := fResults[(frow * fColCount) + I];
